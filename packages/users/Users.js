...
class UsersList extends Component {
  ...
  renderFull($$) {
    ...
    if (items) {
      items.forEach(item => {
        ...
        ).on('change', this._toggleRole.bind(this, item.userId))

        userRoles.forEach(roleId => {
          let option = $$('option').attr({value: roleId}).append(this.getLabel('role-' + roleId))
          if(item.role === roleId) option.attr({selected: 'selected'})
        ...
        grid.append(
          $$(Grid.Row, {user: item}).append(
            ...
            $$(Grid.Cell, {columns: 2}).append(roleSelect),
            // $$(Grid.Cell, {columns: 1}).append(accessCheckbox),
            // $$(Grid.Cell, {columns: 1}).append(superCheckbox),
            $$(Grid.Cell, {columns: 3}).addClass('se-reset').append(resetPwd)
          ).ref(item.userId)
        )
      })
    }
  ...
  _toggleRole(userId, e) {
    const value = e.target.value
    this._toggleAccess(userId, 'role', value)
  }

  _toggleAccess(userId, prop, value) {
